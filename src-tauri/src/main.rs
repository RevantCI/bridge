// Prevents an additional console window on Windows in release builds.
#![cfg_attr(not(debug_assertions), windows_subsystem = "windows")]

mod commands;
mod passage_semantic_wire;
mod sidecar;

use sidecar::EngineSidecar;
use tauri::Manager;

fn main() {
    tauri::Builder::default()
        .plugin(tauri_plugin_shell::init())
        .plugin(tauri_plugin_dialog::init())
        .manage(EngineSidecar::new())
        .setup(|app| {
            let handle = app.handle().clone();
            tauri::async_runtime::spawn(async move {
                let sidecar = handle.state::<EngineSidecar>();
                if let Err(e) = sidecar.start(&handle).await {
                    eprintln!("Failed to start GreekRoomEngine sidecar: {e}");
                }
            });
            Ok(())
        })
        .invoke_handler(tauri::generate_handler![
            commands::engine_ping,
            commands::engine_info,
            commands::engine_log_recent,
            commands::pick_project_folder,
            commands::pick_import_file,
            commands::project_open,
            commands::project_list,
            commands::project_list_book_progress,
            commands::project_forget,
            commands::project_delete,
            commands::project_scan,
            commands::project_report,
            commands::project_collection_report,
            commands::report_generate,
            commands::report_status,
            commands::report_get,
            commands::report_cancel,
            commands::report_export,
            commands::triage_run,
            commands::triage_status,
            commands::triage_cancel,
            commands::triage_override,
            commands::triage_clear,
            commands::triage_results,
            commands::project_inspect_import,
            commands::project_import,
            commands::chapter_verses,
            commands::chapter_verse_data,
            commands::checks_start,
            commands::checks_status,
            commands::checks_cancel,
            commands::checks_retry,
            commands::verse_get,
            commands::verse_run_checks,
            commands::verse_decide,
            commands::verse_edit,
            commands::check_list_for_verse,
            commands::check_validate_selection,
            commands::check_save_selection,
            commands::check_clear_selection,
            commands::issue_resolution_list,
            commands::issue_resolution_save,
            commands::issue_resolution_queue_paratext,
            commands::issue_resolution_retry_paratext,
            commands::alignment_get,
            commands::alignment_status,
            commands::alignment_realign,
            commands::alignment_unalign,
            commands::alignment_save,
            commands::alignment_complete,
            commands::alignment_undo,
            commands::alignment_backups,
            commands::alignment_restore,
            commands::lexicon_get_entry,
            commands::alignment_ai_propose,
            commands::alignment_ai_apply_proposal,
            commands::ai_explain,
            commands::ai_review_start,
            commands::ai_review_status,
            commands::ai_review_cancel,
            commands::ai_review_retry,
            commands::ai_review_list_chapter,
            commands::semantic_validation_list,
            commands::semantic_validation_decide,
            commands::passage_semantic_status,
            commands::passage_semantic_project_metadata,
            commands::passage_semantic_current_passage,
            commands::passage_semantic_stale_summary,
            commands::passage_semantic_migration_report,
            commands::passage_semantic_rebuild_passage,
            commands::source_semantic_build_range,
            commands::source_semantic_get_range,
            commands::source_semantic_get_unit,
            commands::source_semantic_get_coverage_accounts,
            commands::source_semantic_get_diagnostics,
            commands::target_semantic_build_range,
            commands::target_semantic_get_range,
            commands::target_semantic_get_unit,
            commands::target_semantic_get_diagnostics,
            commands::target_semantic_get_search_spans,
            commands::target_semantic_get_capabilities,
            commands::semantic_location_run_range,
            commands::semantic_location_status,
            commands::semantic_location_get_range,
            commands::semantic_location_get_relationship,
            commands::semantic_location_get_candidates,
            commands::semantic_location_get_diagnostics,
            commands::meaning_analysis_run_range,
            commands::meaning_analysis_status,
            commands::meaning_analysis_get_range,
            commands::meaning_analysis_get_assessment,
            commands::meaning_analysis_get_components,
            commands::meaning_analysis_get_diagnostics,
            commands::qa_audit_run_range,
            commands::qa_audit_status,
            commands::qa_audit_get_range,
            commands::qa_audit_get_source_coverage,
            commands::qa_audit_get_target_support,
            commands::qa_audit_get_finding,
            commands::qa_audit_get_diagnostics,
            commands::qa_review_get_queue,
            commands::qa_review_get_finding,
            commands::qa_review_decide_finding,
            commands::qa_review_add_note,
            commands::correction_get_eligibility,
            commands::correction_get_review_context,
            commands::correction_get_proposal,
            commands::correction_list_for_finding,
            commands::correction_create_proposal,
            commands::correction_edit_proposal,
            commands::correction_reject_proposal,
            commands::correction_regenerate_proposal,
            commands::correction_get_proposal_history,
            commands::correction_apply_proposal,
            commands::correction_get_application_status,
            commands::correction_reanalyze_affected,
            commands::correction_verify_application,
            commands::correction_get_verification,
            commands::correction_acknowledge_corrected,
            commands::semantic_review_decide_location,
            commands::semantic_review_decide_meaning,
            commands::review_history_get_entity_history,
            commands::analysis_job_start,
            commands::analysis_job_status,
            commands::analysis_job_cancel,
            commands::analysis_job_get_recent,
            commands::analysis_job_get_scope_status,
            commands::paratext_get_state,
            commands::paratext_set_reference,
            commands::logos_get_state,
            commands::logos_set_reference,
            commands::navigation_status,
            commands::navigation_poll,
            commands::navigation_bridge_changed,
            commands::navigation_resolve,
            commands::settings_get,
            commands::settings_set,
            commands::pick_save_path,
            commands::export_aligned,
            commands::export_non_aligned,
        ])
        .build(tauri::generate_context!())
        .expect("error while running translationCore AI Bridge")
        // The sidecar is built to outlive any single request and nothing used
        // to stop it, so every closed window (and every Ctrl+C'd `tauri dev`)
        // left a bridge-engine.exe running. Besides the stray process, it kept
        // an open handle on the executable, which made the *next* build fail
        // inside tauri-build with a bare "Access is denied" as it tried to
        // refresh target/'s copy of that binary.
        .run(|app, event| {
            if matches!(event, tauri::RunEvent::Exit) {
                app.state::<EngineSidecar>().shutdown(app);
            }
        });
}
