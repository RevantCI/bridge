...
| **v0.8.0-beta.10** | Greek-Room-only live checks bypass translation-help preparation locking so status/list remain responsive on first open; edited verses explicitly show stale AI review with a one-click rerun action. | Source gates pass; installed GUI acceptance pending |
