...
| **v0.8.0-beta.12 / Milestone 3B.4** | Persisted tN/tW issue resolution with exact target text, correction, evidence and reviewer note; crash-safe idempotent Paratext Notes handoff; live project-identity-gated note creation; automatic stale/recheck/resolved/reflagged lifecycle with append-only audit; explicit Advanced-mode acceptance of AI proposals. | Implementation, source gates and local manual lifecycle acceptance complete; installed Beta 12 acceptance pending |
