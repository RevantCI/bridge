...
| **Milestone 3B.4 (in progress)** | Persisted tN/tW issue resolution with exact target text, correction, evidence and reviewer note; explicit crash-safe, idempotent Paratext Notes handoff queue; connector/project identity safety gates. | Initial vertical slice implemented and source-tested; live `create_note` support and automatic edit/recheck closure remain |
