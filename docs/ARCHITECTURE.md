...
| **v0.8.0-beta.11** | AI job progress, completion and errors are scoped to their verse/chapter/project; off-reference jobs remain background work; Translation Helps reserves a stable loading surface during navigation. | Source gates pass; installed GUI acceptance pending |
| **Milestone 3B.4** | Persisted tN/tW issue resolution with exact target text, correction, evidence and reviewer note; crash-safe idempotent Paratext Notes handoff; live project-identity-gated note creation; automatic stale/recheck/resolved/reflagged lifecycle with append-only audit. | Implementation and source gates complete; installed automatic-lifecycle acceptance pending |
