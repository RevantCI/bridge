...
| 4 | Language QA as a check-framework stage; reports, exception queue, publication gate; collection runner | In progress. 4.1 done (A67): a check-job stage, counted in the rollup, persisted in `language_qa_cache` (workbench v4). A decision rescans nothing, and a live edit rescans one verse. 4.2 done (A68): reports, exception queue, publication gate. 4.3 done (A69): one review surface. 4.4–4.5 next |
