...
| A04 | Desktop | Rust tests and compilation | Source | PASS — release and test profiles compile; `cargo test` 10 passed, covering the Beta 7 interactive-timeout guard, the passage-semantic wire enums/offsets, and the sidecar shutdown's forced process-tree kill (both the live-process and already-exited results, since the shutdown log line distinguishes them) |
