...
| A45 | LQA-2 termbase v1, explicit exact-match deprecated-form consistency (items 20-21/50, #169) | Preferred/allowed forms clean; explicit deprecated form flagged (`terminology.deprecated-form`, high severity, distinct from wordlist audit's low-confidence tier); unlisted/unknown forms never called wrong; only `status=="approved"` entries are ever authoritative (provisional/imported and pre-existing no-status payloads inert); multi-word phrase matching; raw span vs. NFC comparison; deterministic finding id and conflict resolution; per-chapter cache invalidated by termbase change, not just by chapter edit; storage round-trip and restart persistence via a real `TranslationCoreProject`; no-termbase and malformed-loader graceful degradation | Source | PASS — 26 new focused engine tests (`test_terminology.py`): 10 pure-function, 5 storage/persistence, 8 `LanguageQaManager` integration, 1 real-project end-to-end. Full engine suite 1160 passed / 1 skipped (up from 1134). Synthetic performance check only (300 concepts / 602 rejected forms against 30,000 generated verses, 0.048 ms/verse) — **no real Tamil termbase or reviewed book available to measure an actual false-positive rate**. No SQL schema migration (`WORKBENCH_SCHEMA_VERSION` unchanged at 3; payload-only `schemaVersion` 1->2). Backend/storage/domain only — no frontend, no curation UI, no frozen-build changes in this slice. **Desktop acceptance NOT YET RUN** — awaiting the maintainer testing the actual build. |
