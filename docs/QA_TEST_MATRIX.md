...
| M45 | Alignment glyph (#70): confirm the per-row indicator always shows `⇄`, colored by completion (grey/amber/green/red), clicking it opens the same Align Words modal the review panel's own button opens for that row's verse, and it visibly disables while a background check is running | PASS — installed acceptance against `Bridge_V1.1-acceptance-01_x64-setup.exe`, 2026-09-12 (`docs/V1_1_ACCEPTANCE_01.md`) |
| M46 | Verse right-click menu (#69): right-click a verse with no findings opens **AI review ▸ Verse/Chapter/Book** and **Edit verse**; a finding's underlined span still opens only its own Accept/Ignore menu; the submenu stays fully on-screen and the parent item stays visibly highlighted while it's open; choosing a scope starts the same AI review the review panel's own buttons start | PASS — installed acceptance against `Bridge_V1.1-acceptance-01_x64-setup.exe`, 2026-09-12 (`docs/V1_1_ACCEPTANCE_01.md`); submenu edge-positioning and the highlighted-parent state were verified as part of the overall walkthrough rather than itemized individually |
| M47 | Verse menu keyboard access (#69): Shift+F10 on a verse with no findings opens the same menu at the row (extends M37/M38's mechanism to a verse with nothing to right-click before this); Escape closes it and returns focus to the row; every item disables under the same conditions as the review panel's own buttons | PASS — installed acceptance against `Bridge_V1.1-acceptance-01_x64-setup.exe`, 2026-09-12 (`docs/V1_1_ACCEPTANCE_01.md`) |
