...
| A38 | Finding context menu | Right-click and keyboard invocation in both the verse editor and the QA review queue; apply-fix disabled (not hidden) with a reason when no proposal exists; decisions routed through the same `decideLocalFinding`/`decideFinding` the panels use | Frontend | PASS — 13 Vitest tests across `FindingContextMenu.test.ts` (3), `VerseList.test.ts` (7, including Shift+F10, the Menu key, arrow navigation between a verse's findings, and the no-finding case), `QaFindingList.test.ts` (1), `AlignmentQaMode.test.ts` (1) and `accessibility.test.ts` (1 — apply-fix reachable with no pointer); svelte-check 0/0. Escape/outside-click dismissal and viewport clamping are unit-tested in `FindingContextMenu.test.ts` but jsdom does not lay out — near-edge positioning needs M37 |
...
| M40 | Choose **Apply proposed fix** on a Greek Room finding that carries a correction; confirm the verse text changes, the verse re-checks, and the corrected finding's underline clears rather than reappearing — including after navigating away and back, and after reopening the project | NOT RUN — `edit_verse` deliberately does not invalidate the USFM/names caches (documented limitation), so the underline clears via the accept path in `ReviewPanel`'s save hook rather than because the finding stopped being produced. Verify the flash-then-clear behavior and that navigating to another verse mid-save does not strand a stale underline |
| M41 | Vocabulary: make an **Accept finding** decision in the verse editor and an **Accept translation as correct** decision in the QA review queue; confirm each view reports the decision it actually recorded and that the hover hints distinguish the two opposite senses of "accept" | NOT RUN — the two views use different decision models over different data sources; the hints are the only thing that disambiguates them |
...
- Frontend: `svelte-check` reports 0 errors and 0 warnings; production Vite build succeeds;
  `npx vitest run` passes 285 tests across 24 files (279 before the issue #38
  keyboard-parity work added 6).
