...
| *(Stage 9A)* | Human QA review, evidence inspection, disposition workflow | ✅ Done (2026-09-02). Alignment Review is a new top-level surface with Word/Semantic/Passage/QA modes; `qaReview.*`, `semanticReview.*` and `reviewHistory.*` record decisions with optimistic concurrency. Findings are classified only — no correction generation or application, which is Stage 9B. Frontend tests run under Vitest (`npm test`). **Caveat:** nothing in the app produces Stage 5-8 analysis, so the queue is empty on any project that has not been seeded — see the *Flag for Benz* entry at the top of `BUILD_LOG.md`. Use `scripts/seed_review_fixture.py` to get a populated project. |
