...
| *(Stage 9A)* | Human QA review, evidence inspection, disposition workflow | ✅ Done (2026-09-02). Alignment Review is a new top-level surface with Word/Semantic/Passage/QA modes; `qaReview.*`, `semanticReview.*` and `reviewHistory.*` record decisions with optimistic concurrency. Findings are classified only — no correction generation or application, which is Stage 9B. Frontend tests run under Vitest (`npm test`). |
